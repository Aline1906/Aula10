...
const deleteMovie = (request, response) => {
    ...
    const movieFiltrado = moviesJson.find(movie => movie.id == idRequirido)
...
const replaceMovies = (req, res) =>{
    ...
    let filteredMovie = moviesJason.find(movie=>movie.id == requestedId);
    ...
    moviesJason.splice(indice, 1, movieFromBody);
    res.status(200).send({
...
const updateLanguage = (req, res)=>{
    ...
    let filteredLingua = moviesJason.find(movie=>movie.id == requestedId);
